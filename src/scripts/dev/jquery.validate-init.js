  ...
  if (subscribeForm.length) {
    ...
    subscribeForm.validate({
      ...
      errorPlacement: function (error, element) {
        if (element.attr("id") === "js-subscribeEmail") {
          error.insertAfter(subscribeForm.find(".footer__form-wrap"));
        } else if (element.hasClass("check__input")) {
          error.insertAfter(subscribeForm.find(".footer__check"));
        } else {
      ...
      submitHandler: function (form, event) {
        ...
        $.ajax({
          ...
          success: function () {
            subscribeEmail.val("");
            subscribeEmail.blur();
            alert("Вы успешно подписались на новостную рассылку LENNI!");
          },
          error: function () {
